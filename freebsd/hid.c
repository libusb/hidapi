...
int HID_API_EXPORT_CALL hid_get_report_descriptor(hid_device *dev, unsigned char *buf, size_t buf_size)
{
	...
	desc.hgd_maxlen = buf_size;
	desc.hgd_data = (void *)buf;
	error = ioctl(dev->device_handle, HIDRAW_GET_REPORT_DESC, &desc);

	if (error < 0) {
		register_device_error_format(dev, "hid_get_report_descriptor: unable to do ioctl on %s", dev->device_path);
		/* error already registered */
		return error;
	}

	if (desc.hgd_actlen < buf_size) {
		buf_size = desc.hgd_actlen;
	}
