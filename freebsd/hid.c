...
struct hid_device_info  HID_API_EXPORT *hid_enumerate(unsigned short vendor_id, unsigned short product_id)
{
	...
	while(hid_get_next_from_sysctl_iter(&iter)) {
		...
		struct hid_device_info *failed;
		if (!tmp)
			continue;
		if (vendor_id != 0 && vendor_id != tmp->vendor_id) {
			failed = tmp->next;
			tmp->next = NULL;
			hid_free_enumeration(failed);
			continue;
		}
		if (product_id != 0 && product_id != tmp->product_id) {
			failed = tmp->next;
			tmp->next = NULL;
			hid_free_enumeration(failed);
			continue;
...
HID_API_EXPORT struct hid_device_info *HID_API_CALL hid_get_device_info(hid_device *dev) {
	...
	if (dev->device_info) {
		...
		if (realpath(dev->device_path, path_buffer) == NULL) {
			register_device_error(dev, "Failed to resolve device path");
			return NULL;
		}
		if (sscanf(path_buffer, "/dev/hidraw%d", &idx) != 1) {
			register_device_error(dev, "Resolved device path is not a hidraw device");
			return NULL;
		}
