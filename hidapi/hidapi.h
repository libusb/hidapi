		...
		*/
		typedef enum {
			/** Arm the callback and fire it for all matching devices that are
				...
				hid_hotplug_register_callback() itself invokes the callback,
				synchronously on the calling thread, once for each such device,
				before it returns.
		...
		/** @brief Hotplug callback function type.
			...
			distinct from any thread the application creates. The only
			exception is the synthetic enumeration pass requested with
			#HID_API_HOTPLUG_ENUMERATE, which is delivered on the thread
			that calls hid_hotplug_register_callback(), before that call
			returns.
			...
			as it was while connected. The path field matches the one
			reported by the corresponding "arrived" event and may be used to
			correlate the two.
		...
		/** @brief Register a HID hotplug callback function.
			...
			When #HID_API_HOTPLUG_ENUMERATE is set, @p callback_handle
			(when non-NULL) is written before any of the synthetic "arrived"
			events are delivered, and the callback always receives its own
